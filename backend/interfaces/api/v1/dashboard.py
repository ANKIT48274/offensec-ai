...
async def dashboard_overview(
    ...
    if project_id:
        f_res = await session.execute(
            ...
            .where(
                FindingModel.assessment_id == project_id
            )  # simplified — in real impl would join project
        ...
        a_res = await session.execute(
            select(func.count(AssetModel.id)).where(AssetModel.project_id == project_id)
        )
        counts["total_assets"] = a_res.scalar() or 0

        s_res = await session.execute(
            select(func.count(ScanJobModel.id)).where(ScanJobModel.project_id == project_id)
        )
        counts["total_scans"] = s_res.scalar() or 0

        ap_res = await session.execute(
            select(func.count(AttackPathModel.id)).where(AttackPathModel.project_id == project_id)
        )
        counts["attack_paths"] = ap_res.scalar() or 0

    n_res = await session.execute(
        select(func.count(NucleiResultModel.id)).where(NucleiResultModel.severity == "critical")
    )
    counts["critical"] += n_res.scalar() or 0
    h_res = await session.execute(
        select(func.count(NucleiResultModel.id)).where(NucleiResultModel.severity == "high")
    )
