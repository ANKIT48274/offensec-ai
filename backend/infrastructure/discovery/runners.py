...
async def run_katana(urls: list[str], timeout: int = KATANA_TIMEOUT) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            await _terminate_gracefully(proc); _cleanup(out_path)
        ...
        for line in raw.strip().split("\n"):
            try:
                ep = json.loads(line)
                endpoints.append({"url": ep.get("url", ""), "status": ep.get("status"), "tech": ep.get("tech", []), "depth": ep.get("depth", 0)})
...
async def run_ffuf(target_url: str, wordlist: str | None = None, timeout: int = FFUF_TIMEOUT) -> dict[str, Any]:
    if not target_url:
    ...
    os.close(fd)
    cmd = ["ffuf", "-u", f"{target_url.rstrip('/')}/FUZZ", "-w", wordlist_path, "-o", out_path, "-of", "json", "-c", "-t", "50", "-fc", "404"]
    try:
        proc = await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.PIPE)
        try:
            _, stderr = await asyncio.wait_for(proc.communicate(), timeout=timeout)
        ...
        return {"results": [{"url": r.get("url", ""), "status": r.get("status"), "length": r.get("length")} for r in (results if isinstance(results, list) else [])], "count": len(results) if isinstance(results, list) else 0, "error": None}
    except FileNotFoundError:
        _cleanup(out_path);
...
async def run_dns_enum(domain: str, timeout: int = DISCOVERY_TIMEOUT) -> dict[str, Any]:
    ...
    for qtype in ["a", "aaaa", "mx", "ns", "txt", "cname"]:
        try:
            proc = await asyncio.create_subprocess_exec("dig", "+short", "-t", qtype, domain, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.DEVNULL)
            try:
                stdout, _ = await asyncio.wait_for(proc.communicate(), timeout=timeout)
            except asyncio.TimeoutError:
                await _terminate_gracefully(proc); continue
...
async def run_whois(target: str, timeout: int = DISCOVERY_TIMEOUT) -> dict[str, Any]:
    try:
        proc = await asyncio.create_subprocess_exec("whois", target, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.DEVNULL)
        try:
            stdout, _ = await asyncio.wait_for(proc.communicate(), timeout=timeout)
        except asyncio.TimeoutError:
...
async def run_tls_analyze(hostname: str, port: int = 443, timeout: int = DISCOVERY_TIMEOUT) -> dict[str, Any]:
    try:
        import ssl
        import socket
        ...
        sock = await loop.run_in_executor(None, lambda: socket.create_connection((hostname, port), timeout=10))
        ssock = await loop.run_in_executor(None, lambda: context.wrap_socket(sock, server_hostname=hostname))
...
async def run_subdomain_enum(domain: str, wordlist: str | None = None, timeout: int = DISCOVERY_TIMEOUT) -> dict[str, Any]:
    ...
    async def check_sub(sub: str) -> str | None:
        try:
            proc = await asyncio.create_subprocess_exec("dig", "+short", f"{sub}.{domain}", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.DEVNULL)
            try:
