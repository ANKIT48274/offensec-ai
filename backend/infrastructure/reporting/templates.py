...
def executive_template(findings: list[dict[str, Any]], assessment: dict[str, Any] | None = None) -> str:
...
<h2>Top Findings</h2>
"""
    for f in sorted(findings, key=lambda x: {"critical": 0, "high": 1, "medium": 2, "low": 3}.get(x.get("severity"), 99))[:10]:
...
def technical_template(findings: list[dict[str, Any]], assessment: dict[str, Any] | None = None) -> str:
    critical = len([f for f in findings if f.get("severity") == "critical"])
    high = len([f for f in findings if f.get("severity") == "high"])
    total = len(findings)
